#### task defination for rating 10
data "template_file" "rating_temp" {
...
resource "aws_ecs_service" "rating" {
  ...
  network_configuration {
    ...
    security_groups = [aws_security_group.rating-sg.id]
  }
  service_registries {
    registry_arn = aws_service_discovery_service.rating_service.arn
  }
}
